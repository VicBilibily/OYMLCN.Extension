...
{
    /// <summary>
    /// CosCloud 基于 cos-dotnet-sdk-v4.2 2017.09.13
    /// </summary>
    public class CosCloud
    {
        ...
        /// HTTP请求超时时间
        /// </summary>
        protected const int HTTP_TIMEOUT_TIME = 60;
        private int appId;
        ...
        /// <param name="fileName">文件名（不是路径）</param>
        /// <param name="stream"></param>
        /// <param name="parameterDic"></param>
        /// <returns></returns>
        public string UploadStream(string bucketName, string remotePath, string fileName, Stream stream, Dictionary<string, string> parameterDic = null)
        {
            if (stream == null)
                return constructResult(ERRORCode.ERROR_CODE_FILE_NOT_EXIST, "local stream not null");
            if (stream.Length > SLICE_UPLOAD_FILE_SIZE)
                return constructResult(ERRORCode.ERROR_CODE_FILE_NOT_EXIST, "local stream too long");
            ...
            return Upload(bucketName, remotePath, fileName, stream, bizAttribute, insertOnly);
        }
        ...
        {
            var url = generateURL(bucketName, remotePath);
            var sha1 = localPath.GetFileInfo().GetSHA1Hash();
            var data = new Dictionary<string, object>
            {
                { "op", "upload" },
                { "sha", sha1 },
        ...
        /// <param name="stream">本地文件流</param>
        /// <param name="bizAttribute">biz_attr属性</param>
        /// <param name="insertOnly">同名文件是否覆盖</param>
        /// <returns></returns>
        public string Upload(string bucketName, string remotePath, string fileName, Stream stream, string bizAttribute = "", int insertOnly = 1)
        {
            var url = generateURL(bucketName, remotePath);
            var sha1 = stream.GetSHA1Hash();
            var data = new Dictionary<string, object>
            {
                { "op", "upload" },
                { "sha", sha1 },
            ...
            return httpRequest.SendRequest(url, ref data, HttpMethod.Post, ref header, timeOut, fileName, stream);
        }
        ...
        /// <returns></returns>
        public string SliceUploadData(string bucketName, string remotePath, string localPath, string fileSha, string session, long offset, int sliceSize, string sign)
        {
            var url = generateURL(bucketName, remotePath);
            var data = new Dictionary<string, object>
            {
            ...
            return httpRequest.SendRequest(url, ref data, HttpMethod.Post, ref header, timeOut, localPath, null, offset, sliceSize);
        }
        ...
        /// <param name="insertOnly">是否覆盖同名文件</param>
        /// <returns></returns>
        public string SliceUploadFile(string bucketName, string remotePath, string localPath, string bizAttribute = "", int sliceSize = SLICE_SIZE.SLIZE_SIZE_1M, int insertOnly = 1)
        {
            var fileInfo = localPath.GetFileInfo();
            var fileSha = fileInfo.GetSHA1Hash();
        ...
        /// </summary>
        /// <returns></returns>	
        private long getExpiredTime() => DateTime.Now.ToTimestamp() + SIGN_EXPIRED_TIME;
        ...
        private string generateURL(string bucketName, string remotePath, string prefix) =>
            generateURL(bucketName, remotePath) + HttpUtility.UrlEncode(prefix);
