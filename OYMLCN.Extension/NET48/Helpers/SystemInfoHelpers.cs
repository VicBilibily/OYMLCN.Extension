...
#pragma warning restore 1591
        ...
        /// <returns></returns>
        private static string FormatBytes(this double bytes)
        {
            int unit = 0;
            while (bytes > 1024)
            {
        ...
        public static System.DateTime BootTime()
        {
            ...
            return System.DateTime.Now - TimeSpan.FromMilliseconds(Environment.TickCount & int.MaxValue);
        }
        /// <summary>
        /// 查询计算机系统信息
        /// </summary>
        ...
        public static double GetUsageVirtualMemory()
            => GetCounterValue(MemoryCounter, "Memory", "% Committed Bytes In Use", null);
        /// <summary>
        /// 获取虚拟内存已用大小
        /// </summary>
        /// <returns></returns>
        public static double GetUsedVirtualMemory()
            => GetCounterValue(MemoryCounter, "Memory", "Committed Bytes", null);
        /// <summary>
        /// 获取虚拟内存总大小
        /// </summary>
        /// <returns></returns>
        public static double GetTotalVirtualMemory()
            => GetCounterValue(MemoryCounter, "Memory", "Commit Limit", null);
        #endregion
        ...
        public static double GetTotalPhysicalMemory()
            => QueryComputerSystem("totalphysicalmemory").ConvertToDouble();
        /// <summary>
        /// 获取空闲的物理内存数，单位B
        /// </summary>
        /// <returns></returns>
        public static double GetFreePhysicalMemory()
            => GetCounterValue(MemoryCounter, "Memory", "Available Bytes", null);
        /// <summary>
        /// 获取已经使用了的物理内存数，单位B
        /// </summary>
        /// <returns></returns>
        public static double GetUsedPhysicalMemory()
            => GetTotalPhysicalMemory() - GetFreePhysicalMemory();
        #endregion
            ...
            try
            {
                ...
                foreach (ManagementObject objMgmt in objCS.Get())
                {
                    ...
                    if (null != device)
                    {
                        ...
                        if (null != total)
                        {
                            dic.Add(device.ToString(), FormatBytes(double.Parse(total.ToString()) - free.ToString().ConvertToDouble()));
                        }
                    }
                }
            ...
            try
            {
                ...
                foreach (ManagementObject objMgmt in objCS.Get())
                {
                    ...
                    if (null != device)
                    {
                        var free = objMgmt["FreeSpace"];
                        var total = objMgmt["Size"];
                        if (null != total && total.ToString().ConvertToDouble() > 0)
                            dic.Add(device.ToString(), 1 - free.ToString().ConvertToDouble() / total.ToString().ConvertToDouble());
                    }
                }
                return dic;
            }
