...
namespace OYMLCN.TencentCloud.Common
{
    ...
    class Request
    {
        ...
        public string SendRequest(string url, ref Dictionary<string, object> data, HttpMethod requestMethod,
            ref Dictionary<string, string> header, int timeOut, string localPath = null, Stream fileStream = null, long offset = -1, int sliceSize = 0)
        {
            try
            {
                ...
                if (requestMethod == HttpMethod.Post)
                {
                    ...
                    else
                    {
                        ...
                        if (localPath != null || fileStream != null)
                        {
                            memStream.Write(beginBoundary, 0, beginBoundary.Length);
                            var fileInfo = localPath.GetFileInfo();
                            var fileExists = fileInfo?.Exists ?? false;
                            if (fileExists)
                            ...
                            var headerText = string.Format(filePartHeader, fileInfo?.Name ?? localPath);
