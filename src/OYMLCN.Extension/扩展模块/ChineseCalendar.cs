...
{
    /// <summary>
    /// 中国农历 支持 1900.1.31日起至 2069.12.31日止的数据
    /// </summary>
    /// <remarks>
    ...
    public class ChineseCalendar
    {
        ...
        private int GetChineseLeapMonth(int year)
            => LunarDateArray[year - MinYear] & 0xF;
        /// <summary>
        /// 传回农历 y年闰月的天数
        /// </summary>
        /// <param name="year"></param>
        ...
        private bool IsWeekend()
            => _date.DayOfWeek == DayOfWeek.Saturday || _date.DayOfWeek == DayOfWeek.Sunday;
        #endregion
        ...
        public string ChineseTwentyFourPrevDay
        {
            get
            {
                ...
                for (int i = 24; i >= 0; i--)
                {
                    var num = 525948.76;
                    if (i > 0)
                        num = num * (y - 1900) + STermInfo[i - 1];
                    else
                        num = num * (y - 1900 - 1) + STermInfo[23];
                    var newDate = baseDateAndTime.AddMinutes(num);
                    if (newDate.DayOfYear < _date.DayOfYear || i == 0)
                    {
                        var idx = i - 1;
                        if (i == 0) idx = 23;
                        tempStr = $"{SolarTerm[idx]}[{newDate:yyyy-MM-dd}]";
                        break;
                    }
                }
                return tempStr;
        ...
        public string ChineseTwentyFourNextDay
        {
            get
            {
                ...
                for (int i = 0; i <= 24; i++)
                {
                    var num = 525948.76;
                    if (i < 24)
                        num = num * (y - 1900) + STermInfo[i];
                    else
                        num = num * (y - 1900 + 1) + STermInfo[0];
                    var newDate = baseDateAndTime.AddMinutes(num);
                    if (newDate.DayOfYear > _date.DayOfYear || i == 24)
                    {
                        var idx = i;
                        if (i == 24) idx = 0;
                        tempStr = $"{SolarTerm[idx]}[{newDate:yyyy-MM-dd}]";
